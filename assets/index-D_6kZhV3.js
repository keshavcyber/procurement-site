...
<p>A clear view of your purchases, from request to delivery.</p>
    ...
    ${c.length>1?`<div class="adm-tabs" aria-label="Request scope">
      ...
      ${n?`<button class="adm-tab ${b?"active":""}" data-tab="dept">${o(a.department||"Your department")} <span>${C.length}</span></button><button class="adm-tab ${m?"active":""}" data-tab="approved">Approved by you <span>${B.length}</span></button>`:""}
    ...
    <div class="dash">
      ...
      <div class="vgrid" id="vgrid">${yt(t,pe)}</div>
    </div>`;const a=e.querySelector("#vgrid"),n=e.querySelector("#vendorSearch"),r=e.querySelector(".admSearchClear"),i=()=>{pe=n.value,r.hidden=!pe,a.innerHTML=yt(t,pe)};n.oninput=i,n.onkeydown=c=>{c.key==="Escape"&&n.value&&(n.value="",i())},r.onclick=()=>{n.value="",i(),n.focus()}}function Wa(e,t,s){const a=(t.vendors||[]).find(m=>m.name.toLowerCase()===s.toLowerCase());if(!a){e.innerHTML=`<div class="dash"><div class="card">Vendor not found: <b>${o(s)}</b> — <a href="#/vendors">back to vendors</a></div></div>`;return}const n=tt(t.prs,a.name),r=Bt(a,t.prs),i=t.me&&t.me.role==="admin",c=et(t.prs,a.name).sort((m,$)=>($.createdAt||"").localeCompare(m.createdAt||"")),b=[["Contact person",a.contactPerson],["Phone",a.phone],["Email",a.email],["Website",a.website],["Address",a.address],["Payment terms",a.paymentTerms],["Bank",a.bankName]].filter(([,m])=>m);e.innerHTML=`
    <div class="dash">
      <div class="adm-head">
    ...
    </div>`}).join("")}</div>`}function ft(e,{valueFmt:t=String,width:s=640,height:a=170}={}){if(e.length<2)return'<div class="chart-empty">Not enough months yet to plot a trend.</div>';const n={l:8,r:8,t:14,b:22},r=s-n.l-n.r,i=a-n.t-n.b,c=Math.max(...e.map(p=>p.value),1),b=r/(e.length-1),m=p=>n.l+p*b,$=p=>n.t+i-p/c*i,k=e.map((p,w)=>`${w===0?"M":"L"}${m(w).toFixed(1)} ${$(p.value).toFixed(1)}`).join(" "),P=`${k} L${m(e.length-1).toFixed(1)} ${n.t+i} L${m(0).toFixed(1)} ${n.t+i} Z`,B=[0,.5,1].map(p=>`<line x1="${n.l}" x2="${s-n.r}" y1="${(n.t+i*(1-p)).toFixed(1)}" y2="${(n.t+i*(1-p)).toFixed(1)}" stroke="var(--line)" stroke-width="1"/>`).join(""),C=Math.ceil(e.length/6)||1,L=e.map((p,w)=>w%C===0||w===e.length-1?`<text x="${m(w).toFixed(1)}" y="${a-6}" font-size="9.5" fill="var(--mut)" text-anchor="${w===0?"start":w===e.length-1?"end":"middle"}">${o(p.month.slice(2))}</text>`:"").join(""),N=e.map((p,w)=>`<circle cx="${m(w).toFixed(1)}" cy="${$(p.value).toFixed(1)}" r="3" fill="var(--brand)" stroke="var(--panel)" stroke-width="1.5"><title>${o(p.month)}: ${o(t(p.value))}</title></circle>`).join("");return`<svg viewBox="0 0 ${s} ${a}" class="chart-line" role="img" aria-label="Monthly trend" preserveAspectRatio="xMidYMid meet">
    ${B}
    ...
    ${N}
    ${L}
  </svg>`}const Qa=["Submitted","Approved","Ordered","In Transit","Received","On Hold","Rejected","Cancelled"],Ja={Submitted:"var(--blue)",Approved:"var(--brand)",Ordered:"var(--amber)","In Transit":"var(--amber)",Received:"var(--brand)","On Hold":"var(--mut)",Rejected:"var(--red)",Cancelled:"var(--red)"},Xa={"0–7 days":"var(--brand)","8–14 days":"var(--brand)","15–30 days":"var(--amber)","30+ days":"var(--red)"},Se={currency:""};function Kt(e,t){const s=t.me||{role:"",department:""},a=s.role==="approver",n=a?Et(t.prs,s.department):t.prs||[],r=fa(n);r.includes(Se.currency)||(Se.currency=r[0]||"");const i=Se.currency,c=f=>i?ee(i,f):String(f),b=i?pt(n,"spend",i):[],m=pt(n,"count"),$=i?ga(n,i,6).map(f=>({label:f.vendor,value:f.total})):[],k=!a&&i?ba(n,i).map(f=>({label:f.department,value:f.total})):[],P=$a(n),B=Qa.filter(f=>P[f]).map(f=>({label:f,value:P[f]})),C=Sa(n),L=ka(n),N=L.map(f=>({label:f.label,value:f.count})),p=L.reduce((f,h)=>f+h.count,0),w=b.reduce((f,h)=>f+h.value,0),I=Mt(n);e.innerHTML=`
      ...
      <div class="insights-overview">
        <section class="card spend-card">
          <div class="section-heading"><div><h2>Spend overview</h2><p>Active request value by month${i?" · "+o(i):""}</p></div>
            ${r.length?`<select id="insCur" aria-label="Insights currency">${r.map(f=>`<option value="${o(f)}" ${f===i?"selected":""}>${o(f)}</option>`).join("")}</select>`:""}
          </div>
          <div class="spend-chart">${b.length?ft(b,{valueFmt:f=>ee(i,f),height:180}):`<div class="trend-empty">${v("chart")}<div><b>Your spending story starts here</b><span>Priced requests will appear in this overview.</span></div></div>`}</div>
        </section>
    ...
    <label class="item-field">Unit *<select name="i_unit" required>${ie(Ze(e,"units"),t.unit||"pcs")}</select></label>
          ...
          <div class="pd-body pd-form">
            <div class="pd-grid">
              ...
              <div class="pd-field">${ne("Currency","currency")}
                <input aria-label="Currency" id="curSearch" class="combo" autocomplete="off" spellcheck="false" value="${o(bt(n.currency||"INR"))}">
                <input type="hidden" name="currency" value="${o(n.currency||"INR")}">
                <div class="curList" id="curList" hidden></div>
              </div>
              <label>${ne("Priority","priority",!0)} <select name="priority">${ie(Ze(t,"priorities"),n.priority||"Medium")}</select></label>
        ...
        <div class="card">
          ...
          <div class="pd-body pd-form">
            <div class="pd-grid">
              ...
              <label>Payment term <select name="paymentTerm">${ie(Ze(t,"paymentTerms"),n.paymentTerm||"",!0)}</select></label>
  ...
  <div class="pd-person">
    ...
    <div class="dash detail-page">
      ...
      <div class="adm-head request-heading">
        ...
        <div class="request-actions">
          ...
          ${C?`<button class="btn primary" data-to="${o(C)}">${v(p(C))}${o(N(C))}</button>`:""}
      ...
      <div style="overflow-x:auto">
        <table class="adm-tbl">
          ...
          ${n.map(r=>`<tr class="vRow" data-name="${o(r.name)}"
            data-search="${at(r.name,r.displayName,r.category,r.type,(r.departments||[]).join(" "))}"
    ...
    <div class="adm">
      ...
      ${Y==="users"?fn(t):Y==="vendors"?cn(t,te):gn(t,Tt[Y])}
    </div>`,e.querySelectorAll(".adm-tab").forEach(a=>a.onclick=()=>{Y=a.dataset.tab,te=!1,pn(),le(e,t)}),e.querySelector("#addToggle").onclick=()=>{if(te=!te,le(e,t),te){const a=e.querySelector(".adm-addrow input, .adm-addrow select");a&&a.focus()}},Y==="users"?bn(e,t):Y==="vendors"?un(e,t,()=>{te=!1,le(e,t)}):$n(e,t,Tt[Y])}function fn(e){const t=a=>(he.includes(a.role)?he:[a.role,...he]).map(n=>`<option value="${o(n)}" ${n===a.role?"selected":""} ${n?"":"disabled"}>${n?o(je(n)):"— assign role —"}</option>`).join(""),s=a=>["",...a&&!Te(e).includes(a)?[a,...Te(e)]:Te(e)].map(n=>`<option value="${o(n)}" ${n===(a||"")?"selected":""}>${n?o(n):"— no department —"}</option>`).join("");return`
    ...
    <div class="adm-card">
      ...
      <div style="overflow-x:auto">
        <table class="adm-tbl">
          ...
          <tbody>
          ${[...ce].sort((a,n)=>(a.role?1:0)-(n.role?1:0)).map(a=>{const n=a.name||Ee(a.email);return`<tr data-search="${at(n,a.email)}">
            <td>
              <div class="adm-user">
                <div class="adm-avatar" style="background:${hn(a.email)}">${o(Xe(a.email))}${a.picture?`<img src="${o(a.picture)}" alt="" referrerpolicy="no-referrer" loading="lazy" onerror="this.remove()">`:""}</div>
                <div>
          ...
          ${nt(5,"No member matches that name or email.")}
          </tbody>
        </table>
      </div>
      <div class="adm-foot">
    ...
    </div>`}function bn(e,t){st(e,{get:()=>Qe,set:n=>{Qe=n},count:(n,r)=>`Showing ${n} of ${r} active members`});const s=async(n,r,i)=>{try{const c=await O("userSet",{email:n,...r});ce=c.users,te=!1,await M.applyResult(c),q(i),e.isConnected&&le(e,t)}catch(c){q(c.message,!0)}};e.querySelectorAll(".roleSel").forEach(n=>n.onchange=()=>s(n.dataset.email,{role:n.value},`${n.dataset.email} → ${n.value}`)),e.querySelectorAll(".deptSel").forEach(n=>n.onchange=()=>s(n.dataset.email,{department:n.value},`${n.dataset.email} → ${n.value||"no department"}`)),e.querySelectorAll(".rmBtn").forEach(n=>n.onclick=()=>{confirm(`Are you sure you want to remove ${n.dataset.email}? This action is permanent.`)&&s(n.dataset.email,{role:""},`${n.dataset.email} removed`)});const a=e.querySelector("#addBtn");a&&(a.onclick=()=>{const n=e.querySelector("#newEmail").value.trim(),r=e.querySelector("#newRole").value,i=e.querySelector("#newDept").value;s(n,{role:r,department:i},`${n} → ${r}`)})}function Te(e){const t=e.lists&&e.lists.departments||[],s=(fe||[]).map(a=>a.department);return[...new Set([...t,...s])]}function gn(e,t){const s=[...t.get()].sort((a,n)=>a.department.localeCompare(n.department)||a[t.key].localeCompare(n[t.key]));return`
    <div class="adm-card">
      ${Ie(t.q,`Search ${t.plural} by name or department…`)}
      ${te?`
      <div class="adm-addrow">
        <select id="mpDept" class="adm-select" style="width:auto">
          ${Te(e).map(a=>`<option value="${o(a)}">${o(a)}</option>`).join("")||'<option value="">— no departments —</option>'}
        </select>
      ...
      <div style="overflow-x:auto">
        <table class="adm-tbl">
          ...
          ${nt(3,`No ${t.label.toLowerCase()} matches that name or department.`)}
          </tbody>
        </table>
      </div>
      <div class="adm-foot">
  ...
  </div>`,Yt(e),document.getElementById("btnRefresh").onclick=async()=>{await M.refresh(),M.get().err||q("Data refreshed")};const C=document.getElementById("nPanel"),L=document.getElementById("nBtn"),N=document.getElementById("pMenu"),p=document.getElementById("profileBtn"),w=()=>{C.hidden=N.hidden=!0,L.setAttribute("aria-expanded","false"),p.setAttribute("aria-expanded","false")};L.onclick=()=>{var S;const l=C.hidden;w(),C.hidden=!l,L.setAttribute("aria-expanded",String(l)),l&&b&&(c.forEach(D=>{D.readAt||(D.readAt="now")}),(S=document.querySelector(".nbadge"))==null||S.remove(),O("notifRead").catch(()=>{}))},p.onclick=()=>{const l=N.hidden;w(),N.hidden=!l,p.setAttribute("aria-expanded",String(l))},document.getElementById("btnOut").onclick=ta,document.addEventListener("click",l=>{l.target.closest(".nbell, .profile-wrap")||w()},{signal:r});const I=document.getElementById("sidebar"),j=document.getElementById("workspace"),f=document.getElementById("openNav"),h=document.getElementById("shell"),g=matchMedia("(max-width: 960px)");let y=!1;const A=(l,S=!0)=>{var D;y=g.matches&&l,h.classList.toggle("nav-open",y),I.inert=g.matches&&!y,j.inert=y,document.getElementById("navBackdrop").hidden=!y,f.setAttribute("aria-expanded",String(y)),document.body.classList.toggle("nav-locked",y),y?(D=I.querySelector("nav a"))==null||D.focus():S&&g.matches&&f.focus()};A(!1,!1),f.onclick=()=>A(!0),document.getElementById("closeNav").onclick=()=>A(!1),document.getElementById("navBackdrop").onclick=()=>A(!1),I.querySelectorAll("a").forEach(l=>l.addEventListener("click",()=>A(!1),{signal:r})),g.addEventListener("change",()=>A(!1,!1),{signal:r}),document.addEventListener("keydown",l=>{if(l.key==="Escape"&&(y?A(!1):C.hidden?N.hidden||(w(),p.focus()):(w(),L.focus())),l.key==="Tab"&&y){const S=[...I.querySelectorAll("a, button")],D=S[0],F=S[S.length-1];l.shiftKey&&document.activeElement===D?(l.preventDefault(),F.focus()):!l.shiftKey&&document.activeElement===F&&(l.preventDefault(),D.focus())}},{signal:r});const R=document.getElementById("view");if(document.querySelector(".skip-link").onclick=l=>{l.preventDefault(),R.focus()},!e.lastSync)R.innerHTML=e.err?`<div class="connection-state">${v("info")}<h1>We couldn't load your workspace</h1><p>${o(e.err)}</p><button class="btn primary" id="retryLoad">Try again</button></div>`:`<div class="loading-workspace" role="status" aria-label="Loading workspace"><div class="skeleton skeleton-title"></div><div class="skeleton skeleton-subtitle"></div><div class="loading-tiles">${'<div class="skeleton"></div>'.repeat(4)}</div><div class="skeleton skeleton-table"></div><p>Getting your workspace ready…</p></div>`,(_=document.getElementById("retryLoad"))==null||_.addEventListener("click",()=>M.refresh(),{signal:r});else{a.fn(R,e,s);const l=t+"/"+(s||"");Ct!==l&&Jt(R),Ct=l}}window.addEventListener("hashchange",()=>{Wt(),window.scrollTo({top:0,behavior:"instant"})});let At="",Pt=!1;M.subscribe(e=>{e.err&&e.err!==At&&q(e.err,!0),At=e.err;const t=!Pt&&e.lastSync;if(t&&(Pt=!0),e.lastSync&&(e.loading||e.err)||Zt().name==="new"&&!t&&e.lastSync){Yt(e);return}Wt()});aa(()=>M.refresh());Ne()||wn();
