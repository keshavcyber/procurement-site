(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function s(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(n){if(n.ep)return;n.ep=!0;const r=s(n);fetch(n.href,r)}})();var Qt;const ee=typeof window<"u"?(Qt=window.matchMedia)==null?void 0:Qt.call(window,"(prefers-reduced-motion: reduce)"):null,Le=new Set,Sa="cubic-bezier(.2,.75,.25,1)";var Jt;(Jt=ee==null?void 0:ee.addEventListener)==null||Jt.call(ee,"change",e=>{e.matches&&Le.forEach(t=>t.cancel())});function Ue(e,{duration:t=240,delay:s=0,distance:a=8,fromOpacity:n=0}={}){if(!(e!=null&&e.animate)||ee!=null&&ee.matches)return;const r=e.animate([{opacity:n,transform:`translateY(${a}px)`},{opacity:1,transform:"translateY(0)"}],{duration:t,delay:s,easing:Sa,fill:"backwards"});return r.id="workspace-reveal",Le.add(r),r.finished.then(()=>Le.delete(r),()=>Le.delete(r)),r}function wa(e){if(ee!=null&&ee.matches)return;const t=e.querySelectorAll([".adm-head",".adm-tabs",".dashboard-kpis > .kpi",".insights-filters",".insights-overview > section",".attention-card",".requests-card",".request-progress",".detail-main > .card",".detail-aside > .card",".form-page #prForm > .card",".insights-page > .kpis > .kpi",".insights-page > .card",".insights-page .adm-grid2 > .card",".vcard",".adm > .adm-card",".adm > .adm-banner"].join(","));let s=0;for(const a of[...t].slice(0,16)){const n=a.getBoundingClientRect();n.bottom<=0||n.top>=window.innerHeight||Ue(a,{delay:Math.min(s++*22,154),distance:10})}}const Xt={APP_URL:"https://script.google.com/macros/s/AKfycby5ZM_xx3GisD_5tBWM9USmVoqKf7nC-6zh7fVZAS6HuBT5gr-TtMOGJNqSmtTsNrc/exec",CLIENT_ID:"975636156405-6b56sp8duaqmjg54tnqqhahiioseokqn.apps.googleusercontent.com"},xe="oizom-id-token";let qt=null;function ka(e){try{return JSON.parse(atob(e.split(".")[1])).exp*1e3}catch{return 0}}function He(){const e=localStorage.getItem(xe);return e?ka(e)<Date.now()+3e4?(localStorage.removeItem(xe),null):e:null}function Aa(){const e=He();if(!e)return null;try{const t=e.split(".")[1].replace(/-/g,"+").replace(/_/g,"/"),s=JSON.parse(decodeURIComponent(escape(atob(t))));return{email:s.email||"",name:s.name||"",picture:s.picture||""}}catch{return null}}function Ra(){localStorage.removeItem(xe),window.google&&google.accounts&&google.accounts.id.disableAutoSelect(),location.reload()}function Ca(e){if(qt=e,He()){e();return}dt(()=>{google.accounts.id.initialize({client_id:Xt.CLIENT_ID,hd:"oizom.com",auto_select:!0,callback:t=>{localStorage.setItem(xe,t.credential),qt()}}),google.accounts.id.prompt()})}function dt(e,t=0){if(window.google&&google.accounts)return e();if(t>100){console.error("Google Identity Services failed to load");return}setTimeout(()=>dt(e,t+1),100)}function Ta(e){dt(()=>{google.accounts.id.renderButton(e,{theme:"filled_blue",size:"large",width:280})})}class et extends Error{constructor(t,s={}){super(t),this.name="ApiError",Object.assign(this,s)}}const ea=new Set(["list","me","usersList","health","logTail"]),qa=new Set([404,408,429,500,502,503,504]),Pa=45e3;function La(e){try{const t=new URL(e.url).hostname;if(t==="script.googleusercontent.com")return"Google response service";if(t==="script.google.com")return"Google backend"}catch{}return"procurement server"}function Te(e,{status:t,stage:s="procurement server",kind:a="network"}){const n=ea.has(e),r=t?`HTTP ${t}`:a==="timeout"?"request timed out":a==="response"?"incomplete response":"connection interrupted",i=n?`Could not load data from the ${s} (${r}). Please try syncing again.`:`Could not confirm your change (${r}). Sync and check whether it saved before submitting again.`;return new et(i,{action:e,status:t,stage:s,kind:a,outcomeUnknown:!n,retryable:!t||qa.has(t)})}async function Da(e,t){const s=He();if(!s)throw new et("SIGNED_OUT");let a;try{a=await fetch(Xt.APP_URL,{method:"POST",cache:"no-store",signal:AbortSignal.timeout(Pa),body:JSON.stringify({...t,action:e,token:s})})}catch(i){throw Te(e,{kind:["TimeoutError","AbortError"].includes(i.name)?"timeout":"network"})}const n=La(a);if(!a.ok)throw Te(e,{status:a.status,stage:n,kind:"http"});let r;try{r=await a.json()}catch{throw Te(e,{stage:n,kind:"response"})}if(!r||typeof r.ok!="boolean"||r.ok&&e==="list"&&!Array.isArray(r.prs))throw Te(e,{stage:n,kind:"response"});if(!r.ok)throw new et(r.error||"Request failed",{action:e});return r}async function U(e,t={}){for(let s=0;s<2;s++)try{return await Da(e,t)}catch(a){if(!a.retryable||(console.warn("[Procurement connection]",{action:e,status:a.status,stage:a.stage,kind:a.kind,attempt:s+1}),!ea.has(e)||s===1))throw a;await new Promise(n=>setTimeout(n,800))}}function Na(e,t){const s=Number(e),a=Number(t);return e===""||e==null||t===""||t==null||!isFinite(s)||!isFinite(a)?"":Math.round(s*a*100)/100}function Ea(e){let t=0,s=!1;for(const a of e){const n=Number(a.lineTotal);a.lineTotal!==""&&a.lineTotal!=null&&isFinite(n)&&(t+=n,s=!0)}return s?Math.round(t*100)/100:""}function Ma(e){if(!e.length)return"";const t=e[0].description||"";return e.length>1?`${t} (+${e.length-1} more)`:t}function xa(e){return e.length?e.length===1?[e[0].qty,e[0].unit].filter(Boolean).join(" "):e.length+" items":""}function Pt(e,t){const s={};for(const a of t)(s[a.prId]=s[a.prId]||[]).push(a);for(const a in s)s[a].sort((n,r)=>Number(n.itemNo)-Number(r.itemNo));return e.map(a=>{const n=s[a.id]||[];return{...a,items:n,amount:a.totalAmount,item:Ma(n),qty:xa(n)}})}let B={prs:[],lists:{},vendors:[],projects:[],materialTypes:[],notifications:[],me:null,lastSync:null,err:"",loading:!1};const tt=new Set;let Lt=!1,ye=null,Ye=0;function Ia(e){const t=["prs","items","vendors","projects","materialTypes","notifications"];if(!e||!Array.isArray(e.prs)||t.some(s=>e[s]!=null&&!Array.isArray(e[s]))||!e.me||typeof e.me.email!="string"||typeof e.me.role!="string")throw new Error("The server did not return your workspace data. Please try again.")}function Ze(){tt.forEach(e=>e(B))}const I={get:()=>B,subscribe(e){return tt.add(e),()=>tt.delete(e)},refresh(){return ye||(B={...B,loading:!0},ye=Promise.resolve().then(async()=>{try{let e,t;do t=Ye,e=await U("list");while(t!==Ye);Ia(e),B={prs:Pt(e.prs,e.items||[]),lists:e.lists||{},vendors:e.vendors||[],projects:e.projects||[],materialTypes:e.materialTypes||[],notifications:e.notifications||[],me:e.me,lastSync:new Date,err:"",loading:!1},Lt=!0}catch(e){if(e.message==="SIGNED_OUT"&&Lt){location.reload();return}B={...B,err:e.message,loading:!1}}}).finally(()=>{ye=null,B={...B,loading:!1},Ze()}),Ze(),ye)},async applyResult(e,{itemsChanged:t=!1}={}){Ye++;const s={err:""};let a=!1;if(e.pr&&e.pr.id){const n=B.prs.find(r=>r.id===e.pr.id);if(!Array.isArray(e.items)&&(t||!n))return I.refresh();if(!n||!(Date.parse(n.updatedAt)>Date.parse(e.pr.updatedAt))){const r=(e.items||(n==null?void 0:n.items)||[]).map(m=>({...m,prId:e.pr.id})),i=Pt([e.pr],r)[0];s.prs=n?B.prs.map(m=>m.id===i.id?i:m):[...B.prs,i]}a=!0}e.deleted&&(s.prs=B.prs.filter(n=>n.id!==e.deleted),a=!0);for(const n of["vendors","projects","materialTypes","notifications"])Array.isArray(e[n])&&(s[n]=e[n],a=!0);if(Array.isArray(e.users)){const n=B.me&&e.users.find(r=>r.email.toLowerCase()===B.me.email.toLowerCase());if(B.me&&(!n||!n.role))return I.refresh();n&&(s.me={...B.me,role:n.role,department:n.department}),a=!0}if(!a)return I.refresh();B={...B,...s},Ze()}},Dt={trash:'<path d="M3 6h18M9 6V3h6v3M5 6l1 15h12l1-15M10 10v7M14 10v7"/>',users:'<circle cx="9" cy="7" r="3"/><path d="M3 21v-3a6 6 0 0 1 12 0v3M16 4a3 3 0 0 1 0 6M17 14a5 5 0 0 1 4 5v2"/>',grid:'<rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/>',vendors:'<path d="M3 10h18M5 10v11h14V10M3 10l2-7h14l2 7M9 21v-7h6v7"/>',chart:'<path d="M4 3v17h17M8 15l4-5 4 2 5-7"/>',settings:'<path d="M4 7h16M4 17h16"/><circle cx="9" cy="7" r="3" fill="currentColor" stroke="none"/><circle cx="15" cy="17" r="3" fill="currentColor" stroke="none"/>',plus:'<path d="M12 5v14M5 12h14"/>',refresh:'<path d="M20 7v5h-5M4 17v-5h5M6 7a7 7 0 0 1 12-1l2 3M4 15l2 3a7 7 0 0 0 12-1"/>',bell:'<path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/>',down:'<path d="m6 9 6 6 6-6"/>',right:'<path d="m9 5 7 7-7 7"/>',left:'<path d="m15 5-7 7 7 7"/>',arrow:'<path d="M4 12h16m-6-6 6 6-6 6"/>',search:'<circle cx="10.5" cy="10.5" r="6.5"/><path d="m16 16 5 5"/>',filter:'<path d="M4 7h16M7 12h10M10 17h4"/>',file:'<path d="M14 3H5v18h14V8zM14 3v5h5M8 12h8M8 16h5"/>',clock:'<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>',wallet:'<path d="M20 8V5H5a2 2 0 0 0 0 4h16v11H5a2 2 0 0 1-2-2V7M21 12h-5v5h5"/>',truck:'<path d="M3 5h11v12H3zM14 9h4l3 4v4h-7"/><circle cx="7" cy="18" r="2"/><circle cx="18" cy="18" r="2"/>',check:'<path d="m5 12 4 4L19 6"/>',package:'<path d="m12 3 9 5v9l-9 5-9-5V8zM3 8l9 5 9-5M12 13v9M8 5l9 5"/>',more:'<circle cx="5" cy="12" r="1"/><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/>',edit:'<path d="m15 4 5 5M4 20l5-1L21 7l-5-5L4 14z"/>',close:'<path d="m6 6 12 12M6 18 18 6"/>',menu:'<path d="M4 6h16M4 12h16M4 18h16"/>',logout:'<path d="M9 4H4v16h5M10 12h11m-5-5 5 5-5 5"/>',shield:'<path d="m12 3 8 3v6c0 5-8 9-8 9s-8-4-8-9V6zM8 12l3 3 5-6"/>',pause:'<path d="M8 5v14M16 5v14"/>',info:'<circle cx="12" cy="12" r="9"/><path d="M12 11v6M12 7v.01"/>'};function v(e,t=""){return`<svg class="ico ${t}" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${Dt[e]||Dt.file}</svg>`}const o=e=>e==null?"":String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]);function Ve(e){return`<span class="chip ${o(e)}" data-s="${o(e)}">${o(e)}</span>`}function N(e,t=!1){document.querySelectorAll(".toast").forEach(a=>a.remove());const s=document.createElement("div");s.setAttribute("role",t?"alert":"status"),s.setAttribute("aria-live",t?"assertive":"polite"),s.className="toast"+(t?" err":""),s.innerHTML=`
    <span class="t-ico">${v(t?"info":"check")}</span>
    <div class="t-body">
      <div class="t-title">${t?"Error":"Success"}</div>
      <div class="t-msg"></div>
    </div>
    <button class="t-close" aria-label="Dismiss">&times;</button>`,s.querySelector(".t-msg").textContent=e,s.querySelector(".t-close").onclick=()=>s.remove(),document.body.appendChild(s),setTimeout(()=>s.remove(),t?6e3:4e3)}const X=e=>e?o(String(e).slice(0,10)):"—";function Ke(e){return String(e||"").split("@")[0].split(/[._-]+/).filter(Boolean).map(s=>s[0].toUpperCase()+s.slice(1)).join(" ")||e}function ct(e){const t=String(e||"").split("@")[0].split(/[._-]+/).filter(Boolean);return((t[0]||"u")[0]+(t[1]?t[1][0]:"")).toUpperCase()}const Nt={INR:"₹",USD:"$",GBP:"£",EUR:"€",JPY:"¥",CNY:"¥",AED:"د.إ ",SGD:"S$",AUD:"A$",CAD:"C$",CHF:"CHF ",Unknown:""},De=e=>Nt[e]!=null?Nt[e]:e+" ";function we(e,t){const s=e==="INR"?"en-IN":"en-US";return De(e)+Number(t).toLocaleString(s,{maximumFractionDigits:2})}function Z(e,t){return e==="INR"?t>=1e7?"₹"+(t/1e7).toFixed(2)+" Cr":t>=1e5?"₹"+(t/1e5).toFixed(1)+"L":"₹"+Math.round(t).toLocaleString("en-IN"):t>=1e6?De(e)+(t/1e6).toFixed(2)+"M":t>=1e3?De(e)+(t/1e3).toFixed(1)+"K":De(e)+Math.round(t).toLocaleString("en-US")}const Ae=["Cancelled","Rejected"],Oa=["Ordered","In Transit","Received"],_e=e=>Oa.includes(e.status)&&e.paymentStatus!=="Paid";function Et(e){const t={};for(const s of e){const a=Number(s.amount);if(!s.amount||!isFinite(a))continue;const n=s.currency||"Unknown";t[n]=(t[n]||0)+a}return Object.entries(t).sort((s,a)=>a[1]-s[1])}function Mt(e){const t=e.filter(n=>!Ae.includes(n.status)),s=e.filter(_e),a=e.filter(n=>n.status==="Received").length;return{total:e.length,pending:e.filter(n=>n.status==="Submitted").length,unpaidCount:s.length,unpaidTotals:Et(s),inTransit:e.filter(n=>n.status==="In Transit").length,received:a,receivedPct:Math.round(a/(e.length||1)*100),spendTotals:Et(t)}}const Ie={total:()=>!0,pending:e=>e.status==="Submitted",unpaid:_e,transit:e=>e.status==="In Transit",received:e=>e.status==="Received",spend:e=>!Ae.includes(e.status)};function Ba(e,t){const s=String(t||"").toLowerCase();return e.filter(a=>String(a.requesterEmail||"").toLowerCase()===s)}function xt(e,t){const s=String(t||"").toLowerCase();return e.filter(a=>String(a.approverEmail||"").toLowerCase()===s&&s&&a.status!=="Rejected")}function ta(e,t){const s=String(t||"").toLowerCase();return e.filter(a=>String(a.department||"").toLowerCase()===s)}function Fa(e){return e.filter(_e)}function ja(e){const t={};for(const s of e){const a=String(s.approverEmail||"").toLowerCase();!a||s.status==="Rejected"||(t[a]=(t[a]||0)+1)}return Object.entries(t).map(([s,a])=>({email:s,count:a})).sort((s,a)=>a.count-s.count||s.email.localeCompare(a.email))}function Ua(e){return[...new Set(e.filter(t=>t.amount&&isFinite(Number(t.amount))).map(t=>t.currency||"Unknown"))].sort()}function It(e,t,s){const a={};for(const n of e){const r=String(n.createdAt||"").slice(0,7);if(!/^\d{4}-\d{2}$/.test(r))continue;let i;if(t==="count")i=1;else{if(Ae.includes(n.status)||t==="unpaid"&&n.paymentStatus!=="Unpaid")continue;const m=Number(n.amount);if(!n.amount||!isFinite(m)||(n.currency||"Unknown")!==s)continue;i=m}a[r]=(a[r]||0)+i}return Object.keys(a).sort().map(n=>({month:n,value:a[n]}))}function Ha(e,t){const s={};for(const a of e){if(Ae.includes(a.status)||(a.currency||"Unknown")!==t)continue;const n=Number(a.amount);if(!a.amount||!isFinite(n))continue;const r=a.department||"Unassigned";s[r]=(s[r]||0)+n}return Object.entries(s).map(([a,n])=>({department:a,total:n})).sort((a,n)=>n.total-a.total)}function Va(e,t,s=6){const a={};for(const i of e){if(Ae.includes(i.status)||(i.currency||"Unknown")!==t)continue;const m=Number(i.amount);if(!i.amount||!isFinite(m))continue;const g=i.vendor||"Unspecified";a[g]=(a[g]||0)+m}const n=Object.entries(a).map(([i,m])=>({vendor:i,total:m})).sort((i,m)=>m.total-i.total);if(n.length<=s)return n;const r=n.slice(s).reduce((i,m)=>i+m.total,0);return[...n.slice(0,s),{vendor:"Other",total:r}]}function Ka(e){const t={};for(const s of e)t[s.status]=(t[s.status]||0)+1;return t}function _a(e){const t=(r,i)=>{const m=Date.parse(r),g=Date.parse(i);return isFinite(m)&&isFinite(g)?(g-m)/864e5:null},s=r=>r.length?r.reduce((i,m)=>i+m,0)/r.length:null,a=e.map(r=>r.createdAt&&r.approvedAt?t(r.createdAt,r.approvedAt):null).filter(r=>r!=null&&r>=0),n=e.map(r=>r.poDate&&r.receivedAt?t(r.poDate,r.receivedAt):null).filter(r=>r!=null&&r>=0);return{avgApprovalDays:s(a),approvalSamples:a.length,avgDeliveryDays:s(n),deliverySamples:n.length}}const Ga=[{key:"0-7",label:"0–7 days",min:0,max:7},{key:"8-14",label:"8–14 days",min:8,max:14},{key:"15-30",label:"15–30 days",min:15,max:30},{key:"30+",label:"30+ days",min:31,max:1/0}];function za(e,t=Date.now()){const s=Ga.map(a=>({...a,count:0}));return e.filter(_e).forEach(a=>{const n=Date.parse(a.poDate||a.updatedAt||a.createdAt);if(!isFinite(n))return;const r=(t-n)/864e5;(s.find(i=>r>=i.min&&r<=i.max)||s[s.length-1]).count++}),s}const pe=["Submitted","Approved","Rejected","Ordered","In Transit","Received","Cancelled","On Hold"],aa=["Unpaid","Paid","Partially Paid","FOC / Free"],Oe={Submitted:{Approved:["admin","approver:dept"],Rejected:["admin","approver:dept"],Cancelled:["admin","requester:own"],"On Hold":["admin"]},Approved:{Ordered:["admin"],Cancelled:["admin"],"On Hold":["admin"],Rejected:["admin"],Submitted:["admin"]},Ordered:{"In Transit":["admin"],Received:["admin"],Cancelled:["admin"],"On Hold":["admin"]},"In Transit":{Received:["admin"],"On Hold":["admin"]},"On Hold":{Submitted:["admin"],Approved:["admin"],Ordered:["admin"],Cancelled:["admin"]},Rejected:{Submitted:["admin","requester:own"],Approved:["admin"]},Received:{},Cancelled:{}};function Ya(e,t,s,a,n){const r=(Oe[e]||{})[t];return r?r.some(i=>i==="requester:own"?s==="requester"&&a:i==="approver:dept"?s==="approver"&&n:i===s):!1}function Za(e,t,s,a){return Object.keys(Oe[e]||{}).filter(n=>Ya(e,n,t,s,a))}function Wa(e,t){return!!(Oe[e]&&Oe[e][t])}const Qa=["Submitted","Approved","Rejected"],Ot=["Approved","Ordered","In Transit","Received","Submitted","On Hold","Rejected","Cancelled"],at=()=>({q:"",dept:"",vendor:"",status:"",from:"",to:""}),c={viewer:"",sel:"total",tab:"mine",statuses:["Approved"],page:1,moreFilters:!1,filters:at()},fe=25,Ja={total:"file",pending:"clock",unpaid:"wallet",transit:"truck",received:"package",spend:"chart"};let nt;function Xa(e,t){c.tab=t==="admin"?"all":"dept",t==="admin"&&(c.statuses=e==="pending"?["Submitted"]:[...pe]),c.sel=["pending","unpaid"].includes(e)?e:"total",c.page=1,c.filters={q:"",dept:"",vendor:"",status:e==="pending"?"Submitted":"",from:"",to:""}}function le(e,t,s=!0){const a=document.activeElement,n=a&&e.contains(a)&&a.id?{id:a.id,start:a.selectionStart,end:a.selectionEnd}:null;if(na(e,t),s&&Ue(e.querySelector(".request-table tbody"),{duration:160,distance:3,fromOpacity:.5}),!n)return;const r=e.querySelector("#"+n.id);if(r&&(r.focus(),n.start!=null&&typeof r.setSelectionRange=="function"))try{r.setSelectionRange(n.start,n.end)}catch{}}const Bt=e=>String(e||"").slice(0,10);function en(e){const t=c.filters;return!(t.dept&&e.department!==t.dept||t.vendor&&e.vendor!==t.vendor||t.status&&e.status!==t.status||t.from&&Bt(e.createdAt)<t.from||t.to&&Bt(e.createdAt)>t.to||t.q&&!`${e.id} ${e.item} ${e.vendor} ${e.department}`.toLowerCase().includes(t.q.trim().toLowerCase()))}function na(e,t){clearTimeout(nt),e.innerHTML=`
...
<p>A clear view of your purchases, from request to delivery.</p>
      ...
      <div id="tabBody"></div>
    </div>`,tn(e.querySelector("#tabBody"),e,t)}const ce=e=>e.length?e.map(([t,s])=>Z(t,s)).join(" + "):"—";function tn(e,t,s){var kt,At,Rt,Ct,Tt;const a=s.me||{role:"",email:"",department:""},n=a.role==="approver",r=a.role==="admin",i=a.role==="finance",m=[(kt=a.email)==null?void 0:kt.toLowerCase(),a.role,(At=a.department)==null?void 0:At.toLowerCase()].join("|");c.viewer!==m&&Object.assign(c,{viewer:m,tab:r?"all":"mine",statuses:["Approved"],sel:"total",page:1,moreFilters:!1,filters:at()});const g=n?["mine","dept","approved"]:r?["all","mine"]:i?["mine","payments"]:["mine"];g.includes(c.tab)||(c.tab="mine");const u=r&&c.statuses.length===1&&c.statuses[0]==="Approved",y=c.statuses.length===pe.length,D=c.tab==="dept",T=c.tab==="approved",q=c.tab==="all",A=c.tab==="payments",F=Ba(s.prs,a.email),M=n?xt(s.prs,a.email):[],R=n?ta(s.prs,a.department):[],P=i?Fa(s.prs):[],E=D?R:T?M:q?s.prs:A?P:F,f=r&&!y?E.filter(l=>c.statuses.includes(l.status)):E,b=Mt(f),W=n?R.filter(Ie.pending):[],O=r?Mt(s.prs):n?{pending:W.length,highPriority:W.filter(l=>["high","critical"].includes(String(l.priority||"").trim().toLowerCase())).length}:null,C=u?[{key:"total",n:b.total,l:"Ready to purchase",s:q?"Approved requests across all departments":"Your approved requests"},{key:"spend",n:b.spendTotals.length?Z(...b.spendTotals[0]):"-",l:"Approved value",s:b.spendTotals.length>1?"+ "+ce(b.spendTotals.slice(1)):"Value of requests ready for purchasing"}]:A?[{key:"total",n:b.total,l:"Awaiting payment",s:ce(b.unpaidTotals)},{key:"transit",n:b.inTransit,l:"In transit",s:"trackable shipments",cls:"warn"},{key:"received",n:b.receivedPct+"%",l:"Received",s:b.received+" of "+b.total,cls:"go"},{key:"spend",n:b.spendTotals.length?Z(...b.spendTotals[0]):"—",l:"Total value",s:b.spendTotals.length>1?"+ "+ce(b.spendTotals.slice(1)):""}]:D?[{key:"total",n:b.total,l:"Department PRs",s:a.department?"in "+a.department:""},{key:"pending",n:b.pending,l:"Pending approval",s:"awaiting decision",cls:"warn"},{key:"unpaid",n:b.unpaidCount,l:"Unpaid",s:ce(b.unpaidTotals),cls:"bad"},{key:"transit",n:b.inTransit,l:"In transit",s:"trackable shipments",cls:"warn"},{key:"received",n:b.receivedPct+"%",l:"Received",s:b.received+" of "+b.total,cls:"go"},{key:"spend",n:b.spendTotals.length?Z(...b.spendTotals[0]):"—",l:"Total spend",s:b.spendTotals.length>1?"+ "+ce(b.spendTotals.slice(1)):""}]:[{key:"total",n:b.total,l:T?"Approved PRs":r&&!y?"Selected PRs":q?"All PRs":"Total PRs",s:T?"across all requesters":r&&!y?"Matching your selected statuses":q?"every department":""},...T?[]:[{key:"pending",n:b.pending,l:"Pending approval",s:"awaiting approver",cls:"warn"}],{key:"unpaid",n:b.unpaidCount,l:"Unpaid",s:ce(b.unpaidTotals),cls:"bad"},{key:"transit",n:b.inTransit,l:"In transit",s:"trackable shipments",cls:"warn"},{key:"received",n:b.receivedPct+"%",l:"Received",s:b.received+" of "+b.total,cls:"go"},{key:"spend",n:b.spendTotals.length?Z(...b.spendTotals[0]):"—",l:T?"Approved spend":"Total spend",s:b.spendTotals.length>1?"+ "+ce(b.spendTotals.slice(1)):""}];if(q)for(const l of ja(f))C.push({key:"ap:"+l.email,n:l.count,l:"Approved by "+Ke(l.email),s:l.email,cls:"go"});C.some(l=>l.key===c.sel)||(c.sel="total");const _=(c.sel.startsWith("ap:")?xt(f,c.sel.slice(3)):f.filter(Ie[c.sel])).sort((l,w)=>(w.createdAt||"").localeCompare(l.createdAt||"")),Q=C.find(l=>l.key===c.sel),h=[...new Set(f.map(l=>l.department).filter(Boolean))].sort(),$=[...new Set(f.map(l=>l.vendor).filter(Boolean))].sort();c.filters.dept&&!h.includes(c.filters.dept)&&(c.filters.dept=""),c.filters.vendor&&!$.includes(c.filters.vendor)&&(c.filters.vendor="");const k=_.filter(en),p=Object.values(c.filters).some(Boolean),d=Math.max(1,Math.ceil(k.length/fe));c.page=Math.min(Math.max(1,c.page),d);const S=k.slice((c.page-1)*fe,c.page*fe),x=["dept","vendor","from","to"].filter(l=>c.filters[l]).length,j=y?"All statuses":c.statuses.join(" + "),G=r?(q?y?"All requests":u?"Approved requests":j:"Your requests")+(q?"":" · "+j):D?"Department requests":T?"Approved by you":A?"Payment queue":"Your requests",se=(l,w,L)=>`<button type="button" id="scope-${l}" class="adm-tab ${c.tab===l?"active":""}" data-tab="${l}" aria-pressed="${c.tab===l}">${w} <span>${L}</span></button>`,Re=l=>String(l.department||"").toLowerCase()===String(a.department||"").toLowerCase(),Ce=l=>{const w=r?pe:n&&l.status==="Submitted"&&Re(l)?Qa:null;return w?`<select class="status-sel" data-status="${o(l.status)}" aria-label="Status for ${o(l.id)}" data-id="${o(l.id)}">${w.map(L=>`<option ${L===l.status?"selected":""}>${o(L)}</option>`).join("")}</select>`:Ve(l.status)},oe=l=>`<select class="pay-sel" aria-label="Payment status for ${o(l.id)}" data-id="${o(l.id)}">${aa.map(w=>`<option ${w===l.paymentStatus?"selected":""}>${o(w)}</option>`).join("")}</select>`,te=r?`<section class="admin-view-bar" aria-label="Admin request view">
      <div class="view-control-row"><span class="view-control-label" id="statusPillLabel">STATUS</span><div class="status-pills" role="group" aria-labelledby="statusPillLabel" aria-describedby="statusPillHint">
        ...
        ${Ot.map((l,w)=>`<button type="button" class="view-pill ${!y&&c.statuses.includes(l)?"selected":""}" id="status-pill-${w}" data-admin-status="${o(l)}" aria-pressed="${!y&&c.statuses.includes(l)}">${l==="Submitted"?"Pending approval":o(l)}<span>${E.filter(L=>L.status===l).length}</span></button>`).join("")}
      </div></div>
      ...
      <div class="view-selection-summary"><span class="view-active-dot"></span><span id="adminViewHeading">${o(G)}</span><span class="view-result-count" role="status">${f.length} ${f.length===1?"request":"requests"}</span></div>
    </section>`:"";e.innerHTML=`
    ${!r&&g.length>1?`<div class="adm-tabs" role="group" aria-label="Request scope">
      ${se("mine","Your requests",F.length)}
      ${n?se("dept",o(a.department||"Your department"),R.length)+se("approved","Approved by you",M.length):""}
      ${i?se("payments","Awaiting payment",P.length):""}
    </div>`:""}
    <div class="kpis dashboard-kpis ${u?"approved-kpis":""}" aria-label="Filter requests by summary">${C.filter(l=>!l.key.startsWith("ap:")).map(l=>`
      <button type="button" class="kpi clickable ${l.cls||""} ${l.key===c.sel?"sel":""}" data-key="${o(l.key)}" aria-pressed="${l.key===c.sel}">
        <span class="kpi-top"><span class="l">${o(l.l)}</span>${v(Ja[l.key])}</span>
        <span class="v">${o(String(l.n))}</span><span class="s">${o(l.s||(l.key==="total"?G:"Active request value"))}</span>
      </button>`).join("")}
    </div>
    ...
    <section class="card requests-card" aria-label="Purchase requests" tabindex="-1">
      <div class="section-heading"><div><h2>Purchase requests <span class="count-badge">${k.length}</span></h2><p>${o(G)} · ${c.sel==="total"?"Latest first":o(Q.l)}</p></div><span class="table-hint">Select a request to view details ${v("arrow")}</span></div>
      ...
      <div class="table-scroll"><table class="tbl request-table"><thead><tr>
        ...
        ${S.map(l=>`<tr class="rowlink ${A?"payment-row":""}" data-id="${o(l.id)}">
          ...
          <td class="request-status">${A?oe(l):Ce(l)}</td>
    ...
    </section>`;const ae=l=>{c.tab=l,c.sel="total",c.page=1,r&&(c.filters=at()),le(t,s)};e.querySelectorAll(".adm-tab").forEach(l=>l.onclick=()=>ae(l.dataset.tab));const ft=()=>{c.statuses=[...pe],ae(c.tab),t.querySelector("#showAllRequests").focus()};(Rt=e.querySelector("#showAllRequests"))==null||Rt.addEventListener("click",ft),(Ct=e.querySelector("#emptyAllRequests"))==null||Ct.addEventListener("click",()=>{c.tab="all",ft()}),e.querySelectorAll("[data-admin-status]").forEach(l=>l.onclick=()=>{const w=l.dataset.adminStatus;if(y)c.statuses=[w];else if(!c.statuses.includes(w))c.statuses=Ot.filter(L=>L===w||c.statuses.includes(L));else if(c.statuses.length>1)c.statuses=c.statuses.filter(L=>L!==w);else return;ae(c.tab)}),e.querySelectorAll("[data-admin-scope]").forEach(l=>l.onclick=()=>ae(l.dataset.adminScope)),(Tt=e.querySelector("#resetAdminView"))==null||Tt.addEventListener("click",()=>{c.statuses=["Approved"],ae("all")}),e.querySelectorAll(".kpi.clickable").forEach(l=>l.onclick=()=>{c.sel=l.dataset.key,c.page=1,le(t,s)}),e.querySelectorAll("[data-queue]").forEach(l=>l.onclick=()=>{var L,V;if(!r&&!(n&&l.dataset.queue==="pending"))return;Xa(l.dataset.queue,a.role),le(t,s);const w=t.querySelector(".requests-card");w.focus({preventScroll:!0}),(V=w.scrollIntoView)==null||V.call(w,{block:"start",behavior:(L=window.matchMedia)!=null&&L.call(window,"(prefers-reduced-motion: reduce)").matches?"auto":"smooth"})}),e.querySelectorAll("tr.rowlink").forEach(l=>l.onclick=w=>{w.target.closest("a, select, button")||(location.hash="#/pr/"+l.dataset.id)}),e.querySelector("#dashMoreFilters").onclick=()=>{c.moreFilters=!c.moreFilters,e.querySelector("#advancedFilters").hidden=!c.moreFilters,e.querySelector("#dashMoreFilters").setAttribute("aria-expanded",String(c.moreFilters))};const ze=e.querySelector("#dashApprover");ze&&(ze.onchange=()=>{c.sel=ze.value,c.page=1,le(t,s)});const bt=l=>{var w,L;c.page+=l,le(t,s),(L=(w=t.querySelector(".requests-card")).scrollIntoView)==null||L.call(w,{block:"start"})};e.querySelector("#dashPrev").onclick=()=>bt(-1),e.querySelector("#dashNext").onclick=()=>bt(1);const he=(l,w)=>{c.filters[l]=w,c.page=1,le(t,s)};e.querySelector("#dashQ").oninput=l=>{c.filters.q=l.target.value,c.page=1,clearTimeout(nt),nt=setTimeout(()=>{t.isConnected&&le(t,s,!1)},150)},e.querySelector("#dashDept").onchange=l=>he("dept",l.target.value),e.querySelector("#dashVendor").onchange=l=>he("vendor",l.target.value);const gt=e.querySelector("#dashStatus");gt&&(gt.onchange=l=>he("status",l.target.value)),e.querySelector("#dashFrom").onchange=l=>he("from",l.target.value),e.querySelector("#dashTo").onchange=l=>he("to",l.target.value);const $t=()=>{c.filters={q:"",dept:"",vendor:"",status:"",from:"",to:""},c.page=1,c.sel="total",le(t,s)},St=e.querySelector("#dashFilterClear"),wt=e.querySelector("#emptyClear");St&&(St.onclick=$t),wt&&(wt.onclick=$t),e.querySelectorAll(".status-sel").forEach(l=>{l.onclick=w=>w.stopPropagation(),l.onchange=async()=>{const w=l.dataset.id,L=s.prs.find(z=>z.id===w),V=l.value;if(!(!L||V===L.status)){if((V==="Rejected"||V==="Cancelled")&&!confirm(`Mark ${w} as ${V}?`)){l.value=L.status;return}l.disabled=!0;try{let z;a.role==="admin"&&!Wa(L.status,V)?z=await U("update",{id:w,updates:{status:V}}):z=await U("transition",{id:w,to:V}),N(`${w} → ${V}`),await I.applyResult(z)}catch(z){N(z.message,!0),l.value=L.status,l.disabled=!1}}}}),e.querySelectorAll(".pay-sel").forEach(l=>{l.onclick=w=>w.stopPropagation(),l.onchange=async()=>{const w=l.dataset.id,L=s.prs.find(z=>z.id===w),V=l.value;if(!(!L||V===L.paymentStatus)){l.disabled=!0;try{const z=await U("update",{id:w,updates:{paymentStatus:V}});N(`${w} payment → ${V}`),await I.applyResult(z)}catch(z){N(z.message,!0),l.value=L.paymentStatus,l.disabled=!1}}}})}function mt(e,t){const s=String(t||"").toLowerCase();return e.filter(a=>String(a.vendor||"").toLowerCase()===s)}function ut(e,t){const s=mt(e,t),a=s.filter(Ie.spend),n={};for(const r of a){const i=Number(r.amount);if(!r.amount||!isFinite(i))continue;const m=r.currency||"INR";n[m]=(n[m]||0)+i}return{count:s.length,spendTotals:Object.entries(n).sort((r,i)=>i[1]-r[1]),unpaid:s.filter(Ie.unpaid).length,lastOrder:s.reduce((r,i)=>{const m=String(i.createdAt||"");return/^\d{4}-\d{2}-\d{2}/.test(m)&&m>r?m:r},"")}}function sa(e,t){if(e.type==="Domestic")return"Domestic";if(e.type==="International")return"Foreign";const s=new Set(mt(t,e.name).filter(r=>r.amount&&isFinite(Number(r.amount))).map(r=>r.currency||"INR"));if(!s.size)return"";const a=s.has("INR"),n=[...s].some(r=>r!=="INR");return a&&n?"Mixed":n?"Foreign":"Domestic"}const an=[{key:"name",weight:3},{key:"displayName",weight:3},{key:"category",weight:2},{key:"type",weight:2},{key:"departments",weight:2},{key:"contactPerson",weight:1},{key:"address",weight:1},{key:"paymentTerms",weight:1},{key:"notes",weight:1}],nn={sensor:["pm","gas","module","electrochemical","particulate"],sensors:["pm","gas","module","electrochemical","particulate"],fab:["fabrication","enclosure","sheet metal","machining"],fabrication:["enclosure","sheet metal","machining"],enclosure:["fabrication","sheet metal","machining"],calibration:["nabl","certification","testing"],certification:["nabl","calibration","testing"],electronics:["components","distributor","semiconductor"],components:["electronics","distributor","semiconductor"],local:["india","domestic"],domestic:["india","local"],foreign:["international","import"],import:["international","foreign"]},sn=1,on=.7,oa=.5,rn=.4,ln=.3,dn=4,cn=e=>e.length>=7?2:e.length>=dn?1:0,Be=e=>String(e??"").toLowerCase().trim();function mn(e,t){const s=e[t];return Be(Array.isArray(s)?s.join(" "):s)}function ra(e){return Be(e).split(/[\s,]+/).filter(Boolean)}function un(e,t){if(e===t)return 0;if(Math.abs(e.length-t.length)>2)return 99;let s=Array.from({length:t.length+1},(a,n)=>n);for(let a=1;a<=e.length;a++){const n=[a];for(let r=1;r<=t.length;r++)n[r]=Math.min(s[r]+1,n[r-1]+1,s[r-1]+(e[a-1]===t[r-1]?0:1));s=n}return s[t.length]}function Ft(e,t){if(!e||!t)return 0;const s=e.split(/[^a-z0-9]+/).filter(Boolean);if(s.includes(t))return sn;if(s.some(n=>n.startsWith(t)))return on;if(e.includes(t))return oa;const a=cn(t);return a&&s.some(n=>un(n,t)<=a)?ln:0}function pn(e,t){const s=Ft(e,t);if(s)return s;const a=nn[t];return a&&a.some(r=>r.includes(" ")?e.includes(r):Ft(e,r)>=oa)?rn:0}function vn(e,t){const s=Array.isArray(t)?t:ra(t);if(!s.length)return 0;let a=0;for(const n of s){let r=0;for(const{key:i,weight:m}of an)r=Math.max(r,pn(mn(e,i),n)*m);if(!r)return 0;a+=r}return a}function ia(e,t){const s=ra(t);return s.length?(e||[]).map(a=>({v:a,score:vn(a,s)})).filter(a=>a.score>0).sort((a,n)=>n.score-a.score||Be(a.v.displayName||a.v.name).localeCompare(Be(n.v.displayName||n.v.name))).map(a=>a.v):[...e||[]]}function Ge(e,t,s="admSearch"){return`
    ...
    </div>`}const pt=(...e)=>o(e.filter(Boolean).join(" ").toLowerCase());function vt(e,t){return`<tr class="adm-nomatch" hidden><td colspan="${e}" style="color:var(--adm-on-var)">${o(t)}</td></tr>`}function ht(e,{get:t,set:s,count:a,id:n="admSearch",match:r=null}){const i=e.querySelector("#"+n);if(!i)return;const m=i.closest(".adm-card"),g=m.querySelector(".admSearchClear"),u=()=>hn(m,t(),a,r);i.oninput=()=>{s(i.value),g.hidden=!i.value,u()},i.onkeydown=y=>{y.key==="Escape"&&i.value&&(i.value="",i.oninput())},g.onclick=()=>{i.value="",i.oninput(),i.focus()},u()}function hn(e,t,s,a){const n=t.trim().toLowerCase(),r=[...e.querySelectorAll("tbody tr[data-search]")],i=n&&a?a(n):null;let m=null;r.forEach(y=>{y.hidden=n?i?!i.has(y.dataset.name):!y.dataset.search.includes(n):!1,y.classList.remove("last-visible"),y.hidden||(m=y)}),m&&m.classList.add("last-visible");const g=e.querySelector(".adm-nomatch");g&&(g.hidden=!!m||!r.length);const u=e.querySelector(".adm-count");u&&(u.textContent=s(r.filter(y=>!y.hidden).length,r.length))}let be="";const la={Domestic:"dom",Foreign:"for",Mixed:"mix"},yn=e=>String(e||"").split(/\s+/).filter(Boolean).slice(0,2).map(t=>t[0]).join("").toUpperCase()||"?";function da(e){let t=e.logoUrl||"";if(!t&&e.website){const s=String(e.website).replace(/^https?:\/\//,"").split("/")[0];s&&(t=`https://www.google.com/s2/favicons?domain=${encodeURIComponent(s)}&sz=64`)}return`<span class="vc-logo">${o(yn(e.displayName||e.name))}${t?`<img src="${o(t)}" alt="" loading="lazy" onerror="this.remove()">`:""}</span>`}function fn(e){const t=[...e.bankName?[{t:e.bankName,cls:"bank"}]:[],...(e.departments||[]).map(n=>({t:n,cls:""}))],s=t.slice(0,3),a=t.length-s.length;return s.map(n=>`<span class="vc-chip ${n.cls}">${o(n.t)}</span>`).join("")+(a>0?`<span class="vc-chip more">+${a} more</span>`:"")}function bn(e,t){const s=ut(e.prs,t.name),a=sa(t,e.prs),n=s.spendTotals.length?Z(...s.spendTotals[0])+(s.spendTotals.length>1?" +":""):"—";return`
    <a class="vcard" href="#/vendors/${encodeURIComponent(t.name)}" data-name="${o(t.name)}">
      <div class="vc-top">
        ...
        ${a?`<span class="vc-badge ${la[a]}">${o(a.toUpperCase())}</span>`:""}
      </div>
      <div class="vc-stats">
    ...
    </a>`}const gn=e=>[...e||[]].sort((t,s)=>(t.displayName||t.name).localeCompare(s.displayName||s.name));function jt(e,t){const s=gn(e.vendors),a=t.trim()?ia(s,t):s;return a.length?a.map(n=>bn(e,n)).join(""):s.length?`<div class="card" style="color:var(--mut)">No vendors match “${o(t)}”.</div>`:'<div class="card" style="color:var(--mut)">No vendors yet — an admin can add them in Admin → Vendors.</div>'}function $n(e,t,s){if(s)return Sn(e,t,decodeURIComponent(s));e.innerHTML=`
    <div class="dash">
      <div class="adm-head">
        <div>
          <h1>Vendors</h1>
      ...
      <div class="vgrid" id="vgrid">${jt(t,be)}</div>
    </div>`;const a=e.querySelector("#vgrid"),n=e.querySelector("#vendorSearch"),r=e.querySelector(".admSearchClear"),i=()=>{be=n.value,r.hidden=!be,a.innerHTML=jt(t,be)};n.oninput=i,n.onkeydown=m=>{m.key==="Escape"&&n.value&&(n.value="",i())},r.onclick=()=>{n.value="",i(),n.focus()}}function Sn(e,t,s){const a=(t.vendors||[]).find(u=>u.name.toLowerCase()===s.toLowerCase());if(!a){e.innerHTML=`<div class="dash"><div class="card">Vendor not found: <b>${o(s)}</b> — <a href="#/vendors">back to vendors</a></div></div>`;return}const n=ut(t.prs,a.name),r=sa(a,t.prs),i=t.me&&t.me.role==="admin",m=mt(t.prs,a.name).sort((u,y)=>(y.createdAt||"").localeCompare(u.createdAt||"")),g=[["Contact person",a.contactPerson],["Phone",a.phone],["Email",a.email],["Website",a.website],["Address",a.address],["Payment terms",a.paymentTerms],["Bank",a.bankName]].filter(([,u])=>u);e.innerHTML=`
    <div class="dash">
      <div class="adm-head">
        <div style="display:flex;gap:14px;align-items:center">
          ...
          <div>
            <h1 style="display:flex;gap:10px;align-items:center">${o(a.displayName||a.name)}
              ${r?`<span class="vc-badge ${la[r]}">${o(r.toUpperCase())}</span>`:""}
            </h1>
      ...
      <div class="kpis">
        ...
        <div class="kpi"><div class="v">${n.spendTotals.length?o(Z(...n.spendTotals[0])):"—"}</div><div class="l">Total spend</div>
          <div class="s">${n.spendTotals.length>1?o(n.spendTotals.slice(1).map(([u,y])=>Z(u,y)).join(" + ")):""}</div></div>
    ...
    ${M}
    ${F}
  </svg>`}const An=["Submitted","Approved","Ordered","In Transit","Received","On Hold","Rejected","Cancelled"],Rn={Submitted:"var(--blue)",Approved:"var(--brand)",Ordered:"var(--amber)","In Transit":"var(--amber)",Received:"var(--brand)","On Hold":"var(--mut)",Rejected:"var(--red)",Cancelled:"var(--red)"},Cn={"0–7 days":"var(--brand)","8–14 days":"var(--brand)","15–30 days":"var(--amber)","30+ days":"var(--red)"},Pe={currency:""};function ma(e,t){const s=t.me||{role:"",department:""},a=s.role==="approver",n=a?ta(t.prs,s.department):t.prs||[],r=Ua(n);r.includes(Pe.currency)||(Pe.currency=r[0]||"");const i=Pe.currency,m=f=>i?Z(i,f):String(f),g=i?It(n,"spend",i):[],u=It(n,"count"),y=i?Va(n,i,6).map(f=>({label:f.vendor,value:f.total})):[],D=!a&&i?Ha(n,i).map(f=>({label:f.department,value:f.total})):[],T=Ka(n),q=An.filter(f=>T[f]).map(f=>({label:f,value:T[f]})),A=_a(n),F=za(n),M=F.map(f=>({label:f.label,value:f.count})),R=F.reduce((f,b)=>f+b.count,0),P=g.reduce((f,b)=>f+b.value,0);e.innerHTML=`
    <div class="dash insights-page">
      ...
      <div class="insights-overview">
        <section class="card spend-card">
          ...
          <div class="spend-chart">${g.length?Ut(g,{valueFmt:f=>Z(i,f),height:180}):`<div class="trend-empty">${v("chart")}<div><b>Your spending story starts here</b><span>Priced requests will appear in this overview.</span></div></div>`}</div>
        </section>
      </div>
      ...
      <div class="adm-grid2">
        <div class="card"><h2>Requests by status</h2>
          <div class="pd-body">${qe(q,{colorOf:f=>Rn[f.label]||"var(--mut)"})}</div></div>
        <div class="card"><h2>Unpaid PO aging</h2>
          <div class="pd-body">${qe(M,{colorOf:f=>Cn[f.label]||"var(--brand)"})}</div></div>
      </div>

      <div class="card">
        <h2>Request volume, by month</h2>
        <div class="pd-body">${Ut(u,{valueFmt:f=>f+" PR"+(f===1?"":"s")})}</div>
      </div>
    </div>`;const E=e.querySelector("#insCur");E&&(E.onchange=()=>{var f;Pe.currency=E.value,ma(e,t),(f=e.querySelector("#insCur"))==null||f.focus()})}const ua={BlueDart:e=>`https://www.bluedart.com/tracking?trackFor=0&trackNo=${e}`,DHL:e=>`https://www.dhl.com/in-en/home/tracking.html?tracking-id=${e}`,FedEx:e=>`https://www.fedex.com/fedextrack/?trknbr=${e}`,DTDC:e=>`https://txn.dtdc.com/ctbs-tracking/customerInterface.tr?submitName=showCITrackingDetails&cnNo=${e}`,Delhivery:e=>`https://www.delhivery.com/track-v2/package/${e}`};function pa(e){try{const t=new URL(String(e||"").trim());return["https:","http:"].includes(t.protocol)?t.href:""}catch{return""}}function Tn(e){const t=String(e.trackingNo||"").trim(),s=pa(e.trackingLink)||(t?(ua[e.courier]||(a=>`https://t.17track.net/en#nums=${a}`))(encodeURIComponent(t)):"");return[o(e.courier||""),s?`<a href="${o(s)}" target="_blank" rel="noopener noreferrer">${o(t||"Track shipment")} ↗</a>`:o(t)].filter(Boolean).join(" ")}function qn(e,t=[]){const s=[...new Set([...t,...Object.keys(ua),"India Post"])];return`<label>Courier<input name="courier" list="deliveryCouriers" autocomplete="off" placeholder="Select or enter a courier" value="${o(e.courier)}"></label>
    ...
    <label class="full">Tracking link<input name="trackingLink" type="url" inputmode="url" placeholder="https://..." aria-describedby="trackingLinkHelp" value="${o(e.trackingLink)}">
      <span class="delivery-help" id="trackingLinkHelp">Paste a tracking link, even if you don't have a tracking number.</span></label>`}function Pn(e){return e?(e.value=e.value.trim(),e.setCustomValidity(e.value&&!pa(e.value)?"Enter a full http:// or https:// tracking link.":""),e.reportValidity()):!0}const Ln="1900-01-01",Dn="2100-12-31",Nn="Enter a complete date with a year between 1900 and 2100.";function Se(e){var t;return((t=String(e||"").match(/^\d{4,}-\d{2}-\d{2}/))==null?void 0:t[0])||""}function va(e){const t=[...e.querySelectorAll('input[type="date"]')],s=a=>{a.setCustomValidity(""),(a.validity.badInput||a.validity.rangeUnderflow||a.validity.rangeOverflow)&&a.setCustomValidity(Nn)};return t.forEach(a=>{a.min=Ln,a.max=Dn;for(const n of["input","change","invalid"])a.addEventListener(n,()=>s(a));s(a)}),()=>t.every(a=>(s(a),a.reportValidity()))}const En=aa,Mn={priorities:["Critical","High","Medium","Low"],couriers:["BlueDart","DHL","FedEx","DTDC","India Post","Other"],departments:[],materialTypes:[],paymentTerms:[],units:[]},Ne=(e,t)=>(e.lists&&e.lists[t]&&e.lists[t].length?e.lists[t]:Mn[t])||[],We={project:"Which running project this purchase belongs to. Only your department’s projects are listed — ask an admin to add one if yours is missing.",purpose:"One line on why this purchase is needed, e.g. “Calibration jigs for the EnvizomPro batch”.",vendor:"The vendor you’ll buy from. Search picks from vendors already registered for your department — if yours isn’t listed, just type its name and an admin will be notified to add it.",currency:"Currency of the vendor’s quote. Type to search any world currency by code, name or symbol.",priority:"Critical = must-have immediately, expedite even at extra cost. High = procure as soon as possible. Medium = needed within the normal purchase cycle. Low = procure when budget allows.",expected:"Date you expect the goods to arrive — used for the In-Transit tracking on the dashboard.",payment:"Whether the vendor has been paid. Usually Unpaid when raising the request.",notes:"Anything the approver or purchase team should know — links, context, constraints.",iDesc:"What you’re buying — one line per item, e.g. “ESP32-S3 DevKit N16R8”.",iZoho:"Zoho part number, links the item to Production inventory (e.g. Z-0042).",iType:"Item category for your department — drives reporting. Ask an admin to add a missing type.",iQty:"How many, counted in the unit chosen next to it.",iUnit:"Unit of measure: pcs, kg, m, set…",iPrice:"Price for ONE unit, in the PR’s currency. Line total = qty × unit price; leave blank if unknown.",iLink:"URL of the exact product page / variant you want ordered.",iDoc:"Datasheet or spec document URL, if relevant."},re=(e,t,s)=>`<span class="lblrow">${o(e)}${We[t]?`<span class="hq ${s?"r":""}" tabindex="0" aria-label="${o(We[t])}" data-tip="${o(We[t])}">?</span>`:""}</span>`;function me(e,t,s){const a=t&&!e.includes(t)?[...e,t]:e;return(s?["",...a]:a).map(n=>`<option value="${o(n)}" ${n===t?"selected":""}>${n?o(n):"Select…"}</option>`).join("")}function Ht(e,t={},s=0,a=[],n=!0){return`<div class="itemrow ${n?"":"nz"}" data-i="${s}">
  ...
  </div>`}function Qe(e){return[...e.querySelectorAll(".itemrow:not(.ithead)")].map(t=>{const s=a=>t.querySelector(`[name="${a}"]`).value.trim();return{description:s("i_description"),partNo:s("i_partNo"),materialType:s("i_materialType"),qty:s("i_qty"),unit:s("i_unit"),unitPrice:s("i_unitPrice"),purchaseLink:s("i_purchaseLink"),datasheetDoc:s("i_datasheetDoc"),lineTotal:s("i_lineTotal")}}).filter(t=>t.description)}function xn(e,t,s){const a=s?t.prs.find(h=>h.id===s):null,n=a||{},r=a?n.items||[]:[{}],i=t.me||{role:""},m=["approver","admin","finance"].includes(i.role),g=a?n.department||"":i.department||"",u=(t.projects||[]).filter(h=>h.department.toLowerCase()===g.toLowerCase()).map(h=>h.project),y=(t.vendors||[]).filter(h=>(h.departments||[]).some($=>$.toLowerCase()===g.toLowerCase())),D=h=>{const $=y.find(k=>k.name.toLowerCase()===String(h||"").toLowerCase());return $?$.displayName||$.name:String(h||"")},T=(t.materialTypes||[]).filter(h=>h.department.toLowerCase()===g.toLowerCase()).map(h=>h.materialType),q=g.toLowerCase()==="production";e.innerHTML=`
      ...
      <form id="prForm">
        <div class="card">
          ...
          <div class="pd-body pd-form">
            <div class="pd-grid">
              <label>${re("Project*","project")} <select name="project" required>${me(u,n.project||"",!0)}</select></label>
              <label>${re("Purpose","purpose")} <input name="purpose" value="${o(n.purpose)}"></label>
              <div class="pd-field full">${re("Vendor","vendor")}
              ...
              <div class="pd-field">${re("Currency","currency")}
              ...
              <label>${re("Priority","priority",!0)} <select name="priority">${me(Ne(t,"priorities"),n.priority||"Medium")}</select></label>
              ${a&&i.role==="admin"?"":`<label>${re("Expected delivery","expected")} <input name="expectedDate" type="date" value="${o(Se(n.expectedDate))}"></label>`}
              ${m?`
              <label>${re("Payment status*","payment")} <select name="paymentStatus" required>${me(En,n.paymentStatus||"Unpaid")}</select></label>`:""}
              ${a&&i.role==="admin"?`
            ...
            <label style="margin-top:14px">${re("Notes","notes")} <textarea name="notes" rows="3">${o(n.notes)}</textarea></label>
          </div>
        </div>
        ...
        ${a&&i.role==="admin"?`<div class="card"><h2>Delivery</h2><div class="pd-body pd-form"><div class="pd-grid">${qn(n,Ne(t,"couriers"))}
          <label>${re("Expected delivery","expected")} <input name="expectedDate" type="date" value="${o(Se(n.expectedDate))}"></label>
        </div></div></div>`:""}

        <div class="card">
          <h2>Requested items</h2><p class="form-caption">Add each item with its quantity and quoted price. Fields marked * are required.</p>
          <div class="pd-body pd-form">
            <div id="itemRows">${r.map((h,$)=>Ht(t,h,$,T,q)).join("")}</div>
    ...
    </div>`;const A=e.querySelector("#prForm"),F=va(A),M=e.querySelector("#itemRows"),R=()=>{const h=Qe(A).map(p=>{const d=Na(p.qty,p.unitPrice);return{lineTotal:d!==""?d:p.lineTotal}}),$=Ea(h),k=A.querySelector('[name="currency"]').value||"INR";e.querySelector("#liveTotal").textContent=$===""?"":"Total: "+we(k,$)},P=h=>{h.querySelector(".rmItem").onclick=()=>{M.children.length>1&&(h.remove(),R())},h.querySelectorAll("input, select").forEach($=>$.oninput=R)};[...M.children].forEach(P),R();const E=(h,$,k,{search:p,resolve:d,toLabel:S,allowEmpty:x,onCommit:j})=>{const H=e.querySelector("#"+h),G=e.querySelector("#"+$),se=A.querySelector(`[name="${k}"]`),Re=()=>{j&&j()},Ce=oe=>{const te=p(oe).slice(0,30);G.innerHTML=te.map(ae=>`<div class="curOpt" data-v="${o(ae.value)}"><b>${o(ae.main)}</b> ${o(ae.name||"")}<span>${o(ae.sub||"")}</span></div>`).join("")||'<div class="curEmpty">No match</div>',G.hidden=!1};H.onfocus=()=>{H.select(),Ce("")},H.oninput=()=>Ce(H.value),G.onmousedown=oe=>{oe.preventDefault();const te=oe.target.closest(".curOpt");te&&(se.value=te.dataset.v,H.value=S(te.dataset.v),G.hidden=!0,Re())},H.onblur=()=>setTimeout(()=>{G.hidden=!0;const oe=H.value.trim();if(!oe&&x)se.value="";else{const te=d(oe);te!=null&&(se.value=te)}H.value=S(se.value),Re()},120)};E("curSearch","curList","currency",{search:h=>kn(h).map($=>({value:$.code,main:$.code,name:$.name,sub:$.sym||""})),resolve:h=>{const $=h.split("—")[0].trim().toUpperCase();return wn($)?$:null},toLabel:h=>ot(h),onCommit:R});const f=h=>{const $=String(h||"").trim().toLowerCase();return y.filter(k=>!$||k.name.toLowerCase().includes($)||(k.displayName||"").toLowerCase().includes($)||(k.category||"").toLowerCase().includes($)).sort((k,p)=>(k.displayName||k.name).localeCompare(p.displayName||p.name)).map(k=>({value:k.name,main:k.displayName||k.name,name:k.displayName?k.name:"",sub:k.category||""}))},b=e.querySelector("#venHint"),W=()=>{const h=A.querySelector('[name="vendor"]').value.trim();b.hidden=!h||y.some($=>$.name.toLowerCase()===h.toLowerCase())};E("venSearch","venList","vendor",{search:f,resolve:h=>{const $=y.find(k=>k.name.toLowerCase()===h.toLowerCase()||(k.displayName||"").toLowerCase()===h.toLowerCase());return $?$.name:h},toLabel:h=>D(h),allowEmpty:!0,onCommit:W}),W(),e.querySelector("#addItem").onclick=()=>{M.insertAdjacentHTML("beforeend",Ht(t,{},M.children.length,T,q)),P(M.lastElementChild),Ue(M.lastElementChild)};const O=A.elements.namedItem("trackingLink");O&&(O.oninput=()=>O.setCustomValidity(""));const C=()=>Object.fromEntries([...new FormData(A)].filter(([h])=>!h.startsWith("i_"))),_=C(),Q=JSON.stringify(Qe(A));A.onsubmit=async h=>{h.preventDefault();const $=e.querySelector("#prSave");if($.disabled||!F()||!Pn(O))return;e.querySelectorAll(".pr-save").forEach(S=>{S.disabled=!0,S.innerHTML=v("refresh","spin")+" Saving…"}),$.disabled=!0,$.textContent="Saving…";const k=C(),p=Qe(A),d=JSON.stringify(p)!==Q;try{if(!p.length&&(!a||d))throw new Error("Add at least one item with a description");if(a){const S=Object.fromEntries(Object.entries(k).filter(([x,j])=>j!==_[x]));if(Object.keys(S).length||d){const x=await U("update",{id:n.id,updates:S,...d?{items:p}:{}});await I.applyResult(x,{itemsChanged:d}),N("PR updated")}location.hash="#/pr/"+n.id}else{const S=await U("create",{pr:k,items:p});await I.applyResult(S,{itemsChanged:!0}),N("Created "+S.pr.id),location.hash="#/pr/"+S.pr.id}}catch(S){N(S.message,!0),$.disabled=!1,$.textContent=a?"Save changes":"Submit PR",e.querySelectorAll(".pr-save").forEach(x=>{x.disabled=!1,x.textContent=a?"Save changes":"Submit request"})}}}function Vt(e,t,s,a){const n=String(e||"").trim();if(n)return n;const r=String(t||"").trim().toLowerCase(),i=String(s||"").trim().toLowerCase(),m=String(a||"").trim();return r&&i&&r===i&&m?m:Ke(t)}const K=(e,t)=>`<div class="pd-f"><span class="vc-l">${o(e)}</span><b>${t||"—"}</b></div>`;let ge=!1,Kt=null;const _t=(e,t,s,a)=>`
  <div class="pd-person">
  ...
  </div>`;function rt(e,t,s){const a=t.prs.find(d=>d.id===s);if(!a){e.innerHTML=`<div class="card">PR ${o(s)} not found ${t.prs.length?"":"(still syncing…)"}</div>`;return}Kt!==s&&(ge=!1,Kt=s);const n=t.me||{role:"",email:"",department:""},r=n.role==="admin",i=a.requesterEmail.toLowerCase()===n.email.toLowerCase(),m=["approver","admin","finance"].includes(n.role),g=r||i&&a.status==="Submitted",u=String(a.department||"").toLowerCase()===String(n.department||"").toLowerCase(),y=Za(a.status,n.role,i,u),D=(a.department||"").toLowerCase()==="production",T=r&&a.status==="Approved",q=r&&a.poNo&&!a.zohoPoId,A=T?"":y.find(d=>!["Rejected","Cancelled","On Hold"].includes(d)),F=y.filter(d=>d!==A),M=d=>({Approved:"Approve request","In Transit":"Mark in transit",Received:"Mark received",Submitted:"Mark submitted"})[d]||"Mark "+d.toLowerCase(),R=d=>({Approved:"check","In Transit":"truck",Received:"package","On Hold":"pause",Cancelled:"close",Rejected:"close"})[d]||"arrow",P=["Submitted","Approved","Ordered","In Transit","Received"],E=P.indexOf(a.status),f=(t.vendors||[]).find(d=>String(d.name||"").toLowerCase()===String(a.vendor||"").toLowerCase()),b=a.paymentTerm||f&&f.paymentTerms||"",W=t.lists&&t.lists.paymentTerms||[],O=["",...b&&!W.includes(b)?[b,...W]:W].map(d=>`<option value="${o(d)}" ${d===b?"selected":""}>${d?o(d):"— select —"}</option>`).join("");e.innerHTML=`
    <div class="dash detail-page">
      ...
      <div class="card">
        ...
        <div class="pd-people">
          ${_t("Requested by",Vt(a.requestedByName,a.requesterEmail,a.approverEmail,a.approvedByName),a.requesterEmail,"Created on "+X(a.createdAt))}
          ${a.approverEmail||a.approvedByName?_t("Approved by",Vt(a.approvedByName,a.approverEmail,a.requesterEmail,a.requestedByName),a.approverEmail,a.approvedAt?"on "+X(a.approvedAt):""):""}
        </div>
        </div>
      </div>
      ...
      <div class="card delivery-card">
        ...
        <div class="pd-grid" id="deliveryRead">
          ${K("Expected",X(a.expectedDate))}
          ${K("Received",X(a.receivedAt))}
          ${K("Tracking",Tn(a))}
          ${K("Notes",o(a.notes))}
        </div>
        </div>
      ...
      <div class="card">
        ...
        <div class="pd-body">
        <div class="pd-grid">
          ${K("PO reference",[o(a.poNo),X(a.poDate)].filter(Boolean).join(" · "))}
          ${K("Invoice / order #",[o(a.invoiceNo),X(a.invoiceDate)].filter(Boolean).join(" · "))}
    ...
    </div>`;const C=e.querySelector("#requestMore");e.onclick=d=>{C&&!C.contains(d.target)&&(C.open=!1)},e.onkeydown=d=>{d.key==="Escape"&&(C!=null&&C.open)&&(C.open=!1,C.querySelector("summary").focus())},C==null||C.addEventListener("focusout",d=>{C.contains(d.relatedTarget)||(C.open=!1)}),e.querySelectorAll("[data-to]").forEach(d=>d.onclick=async()=>{const S=d.dataset.to;if((S==="Rejected"||S==="Cancelled")&&!confirm(`Mark ${a.id} as ${S}?`))return;const x=d.innerHTML;e.querySelectorAll("[data-to], #makePoBtn, #zohoPushBtn").forEach(j=>{j.disabled=!0}),d.innerHTML=v("refresh","spin")+" Updating…";try{const j=await U("transition",{id:a.id,to:S});N(a.id+" → "+S),await I.applyResult(j)}catch(j){N(j.message,!0),e.querySelectorAll("[data-to], #makePoBtn, #zohoPushBtn").forEach(H=>{H.disabled=!1}),d.innerHTML=x}});const _=e.querySelector("#makePoBtn");_&&(_.onclick=()=>{var d,S;ge=!0,rt(e,t,s),Ue((d=e.querySelector("#poForm"))==null?void 0:d.closest(".card")),(S=e.querySelector("[name=poNo]"))==null||S.focus()});const Q=e.querySelector("#poCancelBtn");Q&&(Q.onclick=()=>{ge=!1,rt(e,t,s)});const h=e.querySelector("#poForm"),$=h?va(h):null;h&&(h.onsubmit=async d=>{if(d.preventDefault(),!$())return;const S=new FormData(h),x=String(S.get("poNo")||"").trim();if(!x)return;const j=h.querySelector('button[type="submit"]');j.disabled=!0;let H;try{H=await U("update",{id:a.id,updates:{poNo:x,poDate:S.get("poDate")||"",paymentTerm:S.get("paymentTerm")||""}});const G=await U("transition",{id:a.id,to:"Ordered"});N(a.id+" → Ordered (PO "+x+")"),ge=!1,await I.applyResult(G)}catch(G){H&&await I.applyResult(H),N(G.message,!0),j.disabled=!1}});const k=e.querySelector("#zohoPushBtn");k&&(k.onclick=async()=>{k.disabled=!0;try{const{pr:d}=await U("zohoPushPo",{id:a.id});N(a.id+" → Zoho Books PO "+d.zohoPoNumber),await I.applyResult({pr:d})}catch(d){N(d.message,!0),k.disabled=!1}});const p=e.querySelector("#devDelete");p&&(p.onclick=async()=>{if(confirm("Permanently DELETE "+a.id+"? This cannot be undone.")){p.disabled=!0;try{const d=await U("delete",{id:a.id});N(a.id+" deleted"),location.hash="#/",await I.applyResult(d)}catch(d){N(d.message,!0),p.disabled=!1}}})}let Ee=null,ie=null,it="";const In=["Domestic","International"];function yt(e){return Ee===null&&(Ee=e.vendors||[]),Ee}function On(e){const t=e.lists&&e.lists.departments||[],s=yt(e).flatMap(a=>a.departments||[]);return[...new Set([...t,...s])]}const J=(e,t,s,a="")=>`<label class="adm-field">${o(e)}
    <input class="adm-input" name="${t}" value="${o(s||"")}" placeholder="${o(a)}">
  </label>`;function Bn(e,t){const s=yt(e),a=ie&&s.find(r=>r.name.toLowerCase()===ie.toLowerCase());if(a)return Fn(e,a);const n=[...s].sort((r,i)=>r.name.localeCompare(i.name));return`
    <div class="adm-card">
      ...
      <div class="adm-foot"><span class="adm-count">${ha(n.length,n.length)}</span></div>
    </div>`}const ha=(e,t)=>e===t?`Showing ${t} vendor${t===1?"":"s"}`:`Showing ${e} of ${t} vendors`;function Fn(e,t){const s=ut(e.prs,t.name),a=(s.spendTotals.find(([i])=>i==="INR")||["INR",0])[1],n=e.lists&&e.lists.paymentTerms||[],r=["",...t.paymentTerms&&!n.includes(t.paymentTerms)?[t.paymentTerms,...n]:n].map(i=>`<option value="${o(i)}" ${i===(t.paymentTerms||"")?"selected":""}>${i?o(i):"—"}</option>`).join("");return`
    <div class="adm-card" style="padding:24px">
      ...
      <div class="adm-chips" id="vDepts">
        ${On(e).map(i=>`<button class="adm-chip ${(t.departments||[]).some(g=>g.toLowerCase()===i.toLowerCase())?"on":""}" data-dept="${o(i)}">${o(i)}</button>`).join("")}
      </div>
        ...
        <div class="adm-grid2">
          ...
          <label class="adm-field">Type
            <select class="adm-select" name="type">
              ${["",...In].map(i=>`<option value="${o(i)}" ${i===(t.type||"")?"selected":""}>${i?o(i):"—"}</option>`).join("")}
            </select>
          </label>
    ...
    </div>`}function jn(e,t,s){const a=async(u,y,D)=>{try{const T=await U(u,y);Ee=T.vendors,await I.applyResult(T),N(D),e.isConnected&&s()}catch(T){N(T.message,!0)}};ht(e,{get:()=>it,set:u=>{it=u},count:ha,match:u=>new Set(ia(yt(t),u).map(y=>y.name))}),e.querySelectorAll(".vRow").forEach(u=>u.onclick=y=>{y.target.closest(".vRm")||(ie=u.dataset.name,s())}),e.querySelectorAll(".vRm").forEach(u=>u.onclick=()=>{confirm(`Remove vendor "${u.dataset.name}"? PRs keep the name, but it leaves the registry.`)&&a("vendorRemove",{name:u.dataset.name},`${u.dataset.name} removed`)});const n=e.querySelector("#nvAdd");n&&(n.onclick=()=>{const u=e.querySelector("#nvName").value.trim();if(!u){N("Vendor name required",!0);return}ie=u,a("vendorSet",{name:u,updates:{}},`${u} added — fill in the details`)});const r=()=>{ie=null,s()},i=e.querySelector("#vClose");i&&(i.onclick=r);const m=e.querySelector("#vCancel");m&&(m.onclick=r),e.querySelectorAll("#vDepts .adm-chip").forEach(u=>u.onclick=()=>u.classList.toggle("on"));const g=e.querySelector("#vForm");g&&(g.onsubmit=u=>{u.preventDefault();const y={};for(const[T,q]of new FormData(g))y[T]=q.trim();y.departments=[...e.querySelectorAll("#vDepts .adm-chip.on")].map(T=>T.dataset.dept);const D=y.name||ie;a("vendorSet",{name:ie,updates:y},`${D} saved`),ie=D})}function Un(){ie=null}const $e=["admin","approver","finance","requester"],Hn={admin:"Full access to settings, users, PRs, and analytics.",approver:"Can authorize purchase requests and manage procurement fields.",finance:"Sees POs ready for payment and marks them paid — not tied to a department.",requester:"Can create purchase requests and edit own submitted PRs."},Gt=["#d1e5f7","#8cfb85","#d1e5f9","#e4e2e1"];let Y="users",ve=null,lt="",ke=null,Fe=null,ne=!1;const zt={projects:{key:"project",label:"Project",respKey:"projects",plural:"projects",addRoute:"projectAdd",removeRoute:"projectRemove",q:"",get:()=>ke,set:e=>{ke=e},seed:e=>e.projects},types:{key:"materialType",label:"Item type",respKey:"materialTypes",plural:"item types",addRoute:"materialTypeAdd",removeRoute:"materialTypeRemove",q:"",get:()=>Fe,set:e=>{Fe=e},seed:e=>e.materialTypes}};function Vn(e){let t=0;for(const s of e)t=(t*31+s.charCodeAt(0))%Gt.length;return Gt[t]}const Je=e=>e[0].toUpperCase()+e.slice(1),Kn={users:{title:"User & Role Management",desc:"Manage organizational access by assigning roles to team members. Changes are audited and logged for security compliance.",btn:`${v("users")} Add User`},projects:{title:"Department Projects",desc:"Maintain each department’s running projects. The PR form only offers projects listed here.",btn:`${v("plus")} Add Project`},types:{title:"Department Item Types",desc:"Maintain each department’s item types. PR items must use a type listed for the requester’s department.",btn:`${v("package")} Add Item Type`},vendors:{title:"Vendors",desc:"Register vendors, map them to departments, and keep contact, tax and banking details in one place. The PR form only offers a department’s vendors.",btn:`${v("vendors")} Add Vendor`}};function ue(e,t){if(ve===null){e.innerHTML='<div class="card">Loading users…</div>',U("usersList").then(a=>{ve=a.users,ue(e,t)}).catch(a=>{e.innerHTML=`<div class="card">${o(a.message)}</div>`});return}ke===null&&(ke=t.projects||[]),Fe===null&&(Fe=t.materialTypes||[]);const s=Kn[Y];e.innerHTML=`
    <div class="adm">
      <div class="adm-head">
        <div>
          <h1>${s.title}</h1>
      ...
      ${Y==="users"?_n(t):Y==="vendors"?Bn(t,ne):zn(t,zt[Y])}
    </div>`,e.querySelectorAll(".adm-tab").forEach(a=>a.onclick=()=>{Y=a.dataset.tab,ne=!1,Un(),ue(e,t)}),e.querySelector("#addToggle").onclick=()=>{if(ne=!ne,ue(e,t),ne){const a=e.querySelector(".adm-addrow input, .adm-addrow select");a&&a.focus()}},Y==="users"?Gn(e,t):Y==="vendors"?jn(e,t,()=>{ne=!1,ue(e,t)}):Yn(e,t,zt[Y])}function _n(e){const t=a=>($e.includes(a.role)?$e:[a.role,...$e]).map(n=>`<option value="${o(n)}" ${n===a.role?"selected":""} ${n?"":"disabled"}>${n?o(Je(n)):"— assign role —"}</option>`).join(""),s=a=>["",...a&&!Me(e).includes(a)?[a,...Me(e)]:Me(e)].map(n=>`<option value="${o(n)}" ${n===(a||"")?"selected":""}>${n?o(n):"— no department —"}</option>`).join("");return`
      ...
      <div style="overflow-x:auto">
        <table class="adm-tbl">
          ...
          ${[...ve].sort((a,n)=>(a.role?1:0)-(n.role?1:0)).map(a=>{const n=a.name||Ke(a.email);return`<tr data-search="${pt(n,a.email)}">
            <td>
              <div class="adm-user">
                <div class="adm-avatar" style="background:${Vn(a.email)}">${o(ct(a.email))}${a.picture?`<img src="${o(a.picture)}" alt="" referrerpolicy="no-referrer" loading="lazy" onerror="this.remove()">`:""}</div>
                <div>
    ...
    </div>`}function Gn(e,t){ht(e,{get:()=>lt,set:n=>{lt=n},count:(n,r)=>`Showing ${n} of ${r} active members`});const s=async(n,r,i)=>{try{const m=await U("userSet",{email:n,...r});ve=m.users,ne=!1,await I.applyResult(m),N(i),e.isConnected&&ue(e,t)}catch(m){N(m.message,!0)}};e.querySelectorAll(".roleSel").forEach(n=>n.onchange=()=>s(n.dataset.email,{role:n.value},`${n.dataset.email} → ${n.value}`)),e.querySelectorAll(".deptSel").forEach(n=>n.onchange=()=>s(n.dataset.email,{department:n.value},`${n.dataset.email} → ${n.value||"no department"}`)),e.querySelectorAll(".rmBtn").forEach(n=>n.onclick=()=>{confirm(`Are you sure you want to remove ${n.dataset.email}? This action is permanent.`)&&s(n.dataset.email,{role:""},`${n.dataset.email} removed`)});const a=e.querySelector("#addBtn");a&&(a.onclick=()=>{const n=e.querySelector("#newEmail").value.trim(),r=e.querySelector("#newRole").value,i=e.querySelector("#newDept").value;s(n,{role:r,department:i},`${n} → ${r}`)})}function Me(e){const t=e.lists&&e.lists.departments||[],s=(ke||[]).map(a=>a.department);return[...new Set([...t,...s])]}function zn(e,t){const s=[...t.get()].sort((a,n)=>a.department.localeCompare(n.department)||a[t.key].localeCompare(n[t.key]));return`
    <div class="adm-card">
      ...
      <div class="adm-foot">
        <span class="adm-count">${ya(s.length,s.length,t)}</span>
      </div>
    </div>`}const ya=(e,t,s)=>e===t?`Showing ${t} ${s.label.toLowerCase()}${t===1?"":"s"}`:`Showing ${e} of ${t} ${s.label.toLowerCase()}s`;function Yn(e,t,s){ht(e,{get:()=>s.q,set:r=>{s.q=r},count:(r,i)=>ya(r,i,s)});const a=async(r,i,m)=>{try{const g=await U(r,i);s.set(g[s.respKey]),ne=!1,await I.applyResult(g),N(m),e.isConnected&&ue(e,t)}catch(g){N(g.message,!0)}},n=e.querySelector("#mpAdd");n&&(n.onclick=()=>{const r=e.querySelector("#mpDept").value,i=e.querySelector("#mpName").value.trim();a(s.addRoute,{department:r,[s.key]:i},`${r} / ${i} added`)}),e.querySelectorAll(".mpRm").forEach(r=>r.onclick=()=>{const{dept:i,val:m}=r.dataset;confirm(`Remove "${m}" from ${i}?`)&&a(s.removeRoute,{department:i,[s.key]:m},`${m} removed`)})}const je={requester:0,approver:1,finance:1,admin:2};function Zn(e,t){if(!t||!e||!e.minRole)return!0;const s=je[t.role];return s!=null&&s>=je[e.minRole]}const fa=document.getElementById("app"),Xe={"":{fn:na,nav:"Dashboard",icon:"grid"},vendors:{fn:$n,nav:"Vendors",icon:"vendors",minRole:"admin"},insights:{fn:ma,nav:"Insights",icon:"chart",minRole:"approver"},new:{fn:xn,minRole:"requester"},pr:{fn:rt},admin:{fn:ue,nav:"Admin",icon:"settings",minRole:"admin"}};let de,Yt=null;function ba(){const e=location.hash.replace(/^#\/?/,"").split("/");return{name:e[0]||"",param:e[1]||null}}function Wn(){de==null||de.abort(),fa.innerHTML=`<div class="auth-gate">
  ...
  </div>`,Ta(document.getElementById("gsignin"))}function ga(e){const t=document.getElementById("btnRefresh");t&&(t.disabled=e.loading,t.innerHTML=v("refresh",e.loading?"spin":""),t.setAttribute("aria-label",e.loading?"Refreshing data":"Refresh data"));const s=document.getElementById("syncState");s&&(s.classList.toggle("sync-error",!!e.err),s.textContent=e.loading?"Syncing…":e.err?"Sync failed":e.lastSync?"Up to date":"Connecting…",s.title=e.err||(e.lastSync?"Last full refresh: "+new Date(e.lastSync).toLocaleTimeString():""))}function $a(){var h,$,k;const e=I.get(),{name:t,param:s}=ba(),a=Xe[t]||Xe[""],n=((h=e.me)==null?void 0:h.role)||"";if(e.me&&!Zn(a,e.me)){location.hash="#/";return}de==null||de.abort(),de=new AbortController;const r=de.signal,i=Object.entries(Xe).filter(([,p])=>p.nav&&(!p.minRole||je[n]>=je[p.minRole])).map(([p,d])=>`<a href="#/${p}" ${t===p?'aria-current="page"':""} class="${t===p?"active":""}">${v(d.icon)}<span>${d.nav}</span>${t===p?'<span class="nav-dot"></span>':""}</a>`).join(""),m=e.notifications||[],g=m.filter(p=>!p.readAt).length,u=Aa()||{},y=u.email||(($=e.me)==null?void 0:$.email)||"",D=u.name||Ke(y),T=u.picture?`<img class="avatar" src="${o(u.picture)}" alt="" referrerpolicy="no-referrer">`:`<span class="avatar avatar-txt">${o(ct(D))}</span>`,q=a.nav||(t==="new"?s?"Edit request":"New request":"Purchase request");document.title=q+" · Oizom Procurement",fa.innerHTML=`<div class="app-shell" id="shell">
  ...
  </div>`,ga(e),document.getElementById("btnRefresh").onclick=async()=>{await I.refresh(),I.get().err||N("Data refreshed")};const A=document.getElementById("nPanel"),F=document.getElementById("nBtn"),M=document.getElementById("pMenu"),R=document.getElementById("profileBtn"),P=()=>{A.hidden=M.hidden=!0,F.setAttribute("aria-expanded","false"),R.setAttribute("aria-expanded","false")};F.onclick=()=>{var d;const p=A.hidden;P(),A.hidden=!p,F.setAttribute("aria-expanded",String(p)),p&&g&&(m.forEach(S=>{S.readAt||(S.readAt="now")}),(d=document.querySelector(".nbadge"))==null||d.remove(),U("notifRead").catch(()=>{}))},R.onclick=()=>{const p=M.hidden;P(),M.hidden=!p,R.setAttribute("aria-expanded",String(p))},document.getElementById("btnOut").onclick=Ra,document.addEventListener("click",p=>{p.target.closest(".nbell, .profile-wrap")||P()},{signal:r});const E=document.getElementById("sidebar"),f=document.getElementById("workspace"),b=document.getElementById("openNav"),W=document.getElementById("shell"),O=matchMedia("(max-width: 960px)");let C=!1;const _=(p,d=!0)=>{var S;C=O.matches&&p,W.classList.toggle("nav-open",C),E.inert=O.matches&&!C,f.inert=C,document.getElementById("navBackdrop").hidden=!C,b.setAttribute("aria-expanded",String(C)),document.body.classList.toggle("nav-locked",C),C?(S=E.querySelector("nav a"))==null||S.focus():d&&O.matches&&b.focus()};_(!1,!1),b.onclick=()=>_(!0),document.getElementById("closeNav").onclick=()=>_(!1),document.getElementById("navBackdrop").onclick=()=>_(!1),E.querySelectorAll("a").forEach(p=>p.addEventListener("click",()=>_(!1),{signal:r})),O.addEventListener("change",()=>_(!1,!1),{signal:r}),document.addEventListener("keydown",p=>{if(p.key==="Escape"&&(C?_(!1):A.hidden?M.hidden||(P(),R.focus()):(P(),F.focus())),p.key==="Tab"&&C){const d=[...E.querySelectorAll("a, button")],S=d[0],x=d[d.length-1];p.shiftKey&&document.activeElement===S?(p.preventDefault(),x.focus()):!p.shiftKey&&document.activeElement===x&&(p.preventDefault(),S.focus())}},{signal:r});const Q=document.getElementById("view");if(document.querySelector(".skip-link").onclick=p=>{p.preventDefault(),Q.focus()},!e.lastSync)Q.innerHTML=e.err?`<div class="connection-state">${v("info")}<h1>We couldn't load your workspace</h1><p>${o(e.err)}</p><button class="btn primary" id="retryLoad">Try again</button></div>`:`<div class="loading-workspace" role="status" aria-label="Loading workspace"><div class="skeleton skeleton-title"></div><div class="skeleton skeleton-subtitle"></div><div class="loading-tiles">${'<div class="skeleton"></div>'.repeat(4)}</div><div class="skeleton skeleton-table"></div><p>Getting your workspace ready…</p></div>`,(k=document.getElementById("retryLoad"))==null||k.addEventListener("click",()=>I.refresh(),{signal:r});else{a.fn(Q,e,s);const p=t+"/"+(s||"");Yt!==p&&wa(Q),Yt=p}}window.addEventListener("hashchange",()=>{$a(),window.scrollTo({top:0,behavior:"instant"})});let Zt="",Wt=!1;I.subscribe(e=>{e.err&&e.err!==Zt&&N(e.err,!0),Zt=e.err;const t=!Wt&&e.lastSync;if(t&&(Wt=!0),e.lastSync&&(e.loading||e.err)||ba().name==="new"&&!t&&e.lastSync){ga(e);return}$a()});Ca(()=>I.refresh());He()||Wn();
